function propsFeatures = cutFeatures_v4(G, propsFeatures, maxCutOffsetpx, cutOffsetPercent)

numOfFeatures = numel(propsFeatures);

% Extended section body grid for extended surrounding cuts
Ext1 = zeros([maxCutOffsetpx,size(G,[2,3])]);
G_ext = cat(1,Ext1,G,Ext1);
Ext2 = zeros([size(G_ext,1),maxCutOffsetpx,size(G_ext,3)]);
G_ext = cat(2,Ext2,G_ext,Ext2);


%% CUT FEATURES OF BODY 
% multiple types of cut provided. These types were used for experimental
% determining which setup works best. In later feature identification used
% only X and Y .CutTight
...
for featureID = 1:numOfFeatures
    ...
    propsFeatures(featureID).CutX.CutTight         = propsFeatures(featureID).CutX.Cut(:,:,BB(3,1):BB(3,2));        % only feature height
    propsFeatures(featureID).CutX.CutTightHigh     = propsFeatures(featureID).CutX.Cut(:,:,BB(4,1):BB(4,2));        % bottom of supports to top of overhang
    propsFeatures(featureID).CutX.CutTightWide     = propsFeatures(featureID).CutX.CutExt(:,:,[BB(3,1):BB(3,2)]);

    propsFeatures(featureID).CutX.CutTightMaxWidth = G_ext(propsFeatures(featureID).CutX.CutLine+maxCutOffsetpx,...
                                                           [propsFeatures(featureID).CutX.CutBoundaries(1):propsFeatures(featureID).CutX.CutBoundaries(2)+2*maxCutOffsetpx],...
                                                           [BB(3,1):BB(3,2)]);
    propsFeatures(featureID).CutX.CutMaxWidth      = G_ext(propsFeatures(featureID).CutX.CutLine+maxCutOffsetpx,...
                                                           [propsFeatures(featureID).CutX.CutBoundaries(1):propsFeatures(featureID).CutX.CutBoundaries(2)+2*maxCutOffsetpx],...
                                                           [BB(4,1):BB(4,2)]);
    ...
    propsFeatures(featureID).CutY.CutTight         = propsFeatures(featureID).CutY.Cut(:,:,BB(3,1):BB(3,2));        % only feature height
    propsFeatures(featureID).CutY.CutTightHigh     = propsFeatures(featureID).CutY.Cut(:,:,BB(4,1):BB(4,2));        % bottom of supports to top of overhang
    % propsFeatures(featureID).CutY.CutTightExt      = propsFeatures(featureID).CutY.CutExt(:,:,[cutZOffsetBotLim:cutZOffsetUpLim]);
    propsFeatures(featureID).CutY.CutTightWide     = propsFeatures(featureID).CutY.CutExt(:,:,[BB(3,1):BB(3,2)]);

    propsFeatures(featureID).CutY.CutTightMaxWidth = G_ext([propsFeatures(featureID).CutY.CutBoundaries(1):propsFeatures(featureID).CutY.CutBoundaries(2)+2*maxCutOffsetpx],...
                                                        propsFeatures(featureID).CutY.CutLine+maxCutOffsetpx,...
                                                       [BB(3,1):BB(3,2)]);
    propsFeatures(featureID).CutY.CutMaxWidth = G_ext([propsFeatures(featureID).CutY.CutBoundaries(1):propsFeatures(featureID).CutY.CutBoundaries(2)+2*maxCutOffsetpx],...
                                                        propsFeatures(featureID).CutY.CutLine+maxCutOffsetpx,...
                                                       [BB(4,1):BB(4,2)]);    
